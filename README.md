...
24. При восходящем преобразовании *(baseclass = subclass; baseclass **.** ... )* вызываются те методы, которые принадлежат базовому классу, но выводятся переопределенные производный классом. 
